...
class EchallanQuery(models.Model):
    ...
    query_description = models.TextField(verbose_name="Query Description", null=True, blank=True)
    query_date = models.DateTimeField(auto_now_add=True, verbose_name="Query Date")
    issue_type = models.TextField(verbose_name='Issue Type', null=True, blank=True)
    ...
    document_url = models.TextField(verbose_name='Document URL', null=True, blank=True)
    resolution_details = models.TextField(null=True, blank=True, verbose_name="Resolution Details")
    ...
    def save(self, *args, **kwargs):
        if not self.ticket_no:
            ...
            last_ticket = EchallanQuery.objects.filter(ticket_no__startswith=f"ECH-{current_year}").order_by('-id').first()
            last_number = int(last_ticket.ticket_no.split('-')[-1]) if last_ticket else 0
            self.ticket_no = f"ECH-{current_year}-{last_number + 1:03d}"
        super().save(*args, **kwargs)

    def __str__(self):
        return f"Query by {self.user.email if self.user else 'Unknown'} for E-Challan {self.echallan.id if self.echallan else 'Unknown'}"



class Payment(models.Model):
    e_challan = models.OneToOneField(Echallan_main, on_delete=models.CASCADE, related_name="payment")
    ...
    def __str__(self):
        return f"Payment {self.transaction_id} - {self.e_challan.id}"
...
class EchallanVerification(models.Model):
    STATUS_CHOICES = [
        ('pending', 'Pending'),
        ('verified', 'Verified'),
        ('rejected', 'Rejected'),
    ]
    E_CHALLAN_STATUS_CHOICES = [
        ('pending', 'Pending'),
        ('generated', 'Generated'),
    ]
    ...
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='pending')
    echallan_status = models.CharField(max_length=20, choices=E_CHALLAN_STATUS_CHOICES, default='pending')
...
class PoliceOfficer(models.Model):
    """Police Officer Model extending User"""
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name="police_officer")
    badge_number = models.CharField(max_length=50, unique=True)
    station_name = models.CharField(max_length=255,null=True, blank=True)
    assigned_area = models.CharField(max_length=255,null=True, blank=True)
    status = models.BooleanField(default=False)
    rank = models.CharField(max_length=50)

    def __str__(self):
        return f"{self.user.email} - {self.badge_number}"